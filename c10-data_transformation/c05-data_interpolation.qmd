...
#Change column names
city_pop <- city_pop %>%
  rename(year = Sgnyea, city_id = Ctnm_id, Emp_ind = Eect09, Emp_ser = Eect10, Emp_num = Eect05)
skim(city_pop)
```
...
```{r}
new_citypop2 <- city_pop[, c("year", "city_id", "Emp_ser")]
skim(new_citypop2)
```
...
new_citypop <- new_citypop %>%
  mutate(Emp_agri=100-Emp_ind-Emp_ser) 
skim(new_citypop)
```
